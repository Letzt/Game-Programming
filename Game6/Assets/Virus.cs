...
public class Virus : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Vector3 direc = new Vector3(0.0f, 0.0f, 0.0f);
        float dist = Vector3.Distance(fps_player_obj.transform.position, transform.position);

        if (dist < radius_of_search_for_player)
        {
          direc = fps_player_obj.transform.position - transform.position;
          direc /= Vector3.Magnitude(direc);
        }

        transform.Translate(direc * Time.deltaTime * virus_speed);
